...
/*   Updated: 2024/05/17 12:08:17 by omfelk           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*double_redirect_left(char *str, int *start, t_data *lst_data)
{
	...
	lst_data->here_doc = ft_strdup("here_doc");
	lst_data->fd_here_doc = open(text_end_here_doc, O_RDONLY);
	if (lst_data->fd_here_doc == -1)
	{
		perror("file_input here_doc :");
		return (false);
	}
	dup2(lst_data->fd_here_doc, STDIN_FILENO);
	if (lst_data->fd_here_doc == -1)
		printf("error fd here doc\n");
